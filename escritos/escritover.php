...
mysql_select_db($database_tfx, $tfx);
$query_causa = sprintf("SELECT *, operop(DEPORIG) AS DIOR, operpersona(ID) AS HIDXX, IF(mod(ID,2) = 0,'Rolando Manuel Pankow','Carlos Arturo Serravalle') AS SECRE, concat(if(isnull(DIRCALLE),'',DIRCALLE),if((DIRNRO is not null),' N&#176; ',''),if((DIRNRO is not null),DIRNRO,''),if((DIRPISO is not null),'P� ',''),if((DIRPISO is not null),DIRPISO,''),if((DIRDPTO is not null),'Dpto ',''),if((DIRDPTO is not null),DIRDPTO,''),if((DIRMZ is not null),' Mz ',''),if((DIRMZ is not null),DIRMZ,''),if((DIRCS is not null),' Cs ',''),if((DIRCS is not null),DIRCS,''),if((DIRMBK is not null),' Mbk ',''),if((DIRMBK is not null),DIRMBK,'')) AS DIREC, concat('Expte. N&#176; ',`causas`.`EXPTE`,' - A&#241;o ',year(`causas`.`INFFECHA`)) AS EXPTEX, fechames(INFFECHA) INFFECHAX FROM causas WHERE ID = %s", GetSQLValueString($colname_causa, "int"));
...
<body>
  ...
  <table style="width:80%">
    <tr>
      ...
      <th>
        <h4>ESCRITO: <?php echo $row_escrito['ESCNOM']; ?></h4>
      </th>

    </tr>
    <tr>
      <td style="width: 20%; vertical-align: text-top;">
        <div><?php do { ?><div ondragstart="dragStart(event)" draggable="true"
            id="<?php echo $row_comodin['COMODIN']; ?>"
            style="align:right; background-color:lightblue; margin: 2px; padding-left: 5px; border-style: dotted; border-width: 1px; width: auto;"
            title="<?php echo $row_comodin['COMODIN']; ?>"><?php echo $row_comodin['DESCRIPCION']; ?></div>
          <?php } while ($row_comodin = mysql_fetch_assoc($comodin)); ?></div>
      </td>
      <td style="width: 80%; vertical-align: text-top;">
        <form name="formulario" id="formulario" action="<?php echo $editFormAction; ?>" method="POST">
          Nombre del Escrito: <input type="text" name="escnombre" id="escnombre" placeholder="Nombre del Escrito"
            value="<?php echo $row_escrito['ESCNOM']; ?>" style="width: 350px;" required autofocus>
...
<?php echo $row_escrito['CONTEN']; ?>
              ...
              <div class="TabbedPanelsContent">
                Margen:
                <input type="radio" name="margen" value="exa3"
                  <?php echo ($row_escrito['CONTEN2FORMT']=='exa3')?'checked':'' ; ?> /> Izquierdo
                <input type="radio" name="margen" value="exa2"
                  <?php echo ($row_escrito['CONTEN2FORMT']=='exa2')?'checked':'' ; ?> /> Derecho<br>

                <textarea name="area3" style="width: 600px; height: 450px;" ondrop="drop(event)"
                  ondragover="allowDrop(event)" required>
...
var sprytextfield4 = new Spry.Widget.ValidationTextField("sprytextfield4", "date", {
  format: "dd/mm/yyyy",
  isRequired: false
});
