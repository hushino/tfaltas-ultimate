...
mysql_select_db($database_tfx, $tfx);
$query_montosmultas = sprintf("SELECT CONCAT('Art. ',ART,' - ',HIPOTESIS) AS INFX, (sum($colum_UT) * $antemulti) AS SUMUT, $colum_UT*(SELECT DETALLES FROM opciones WHERE IDC=85) AS UTX, ($colum_UT*(SELECT DETALLES FROM opciones WHERE IDC=85)/2) AS UTX50, nrosenletras((sum($colum_UT) * $antemulti)) AS UTLETRAS, nrosenletras((sum($colum_UT) * $antemulti)*3) AS UTX3LET, ((sum($colum_UT) * $antemulti)*3) AS UTX3 FROM infrac WHERE (SELECT INFART FROM causas WHERE ID=%s) LIKE CONCAT('%%',IDINF,'%%')", GetSQLValueString($ex_montosmultas, "int"));
...
mysql_select_db($database_tfx, $tfx);
$query_leyartinc = sprintf("SELECT GERIUNDO, GERIUTIPO, LEYNRO, LEYINC, LEYART, LEYGRAART, LEYGRAINC, CURSOS FROM infrac WHERE (SELECT INFART FROM causas WHERE ID=%s) LIKE CONCAT('%%',IDINF,'%%') ORDER BY GERIUTIPO", GetSQLValueString($ex_lai, "int"));
...
$contador = '0'; $comas = ''; $texto2 = ''; $sumgeriundo = ''; $sumartinc = '' . ''; $sumcurso = ''; $sumcursotext = ' los m&oacute;dulos ';
...
$sumcurso = $sumcurso . $comas . $row_leyartinc['CURSOS'];
...
$pos1 = strpos($sumcurso, '1'); $pos2 = strpos($sumcurso, '2'); $pos3 = strpos($sumcurso, '3'); $pos4 = strpos($sumcurso, '4'); $pos5 = strpos($sumcurso, '5'); $pos6 = strpos($sumcurso, '6'); $pos7 = strpos($sumcurso, '7');

if ($pos1 !== false) {  $sumcursotext = $sumcursotext . ' uno (1), '; }
if ($pos2 !== false) {  $sumcursotext = $sumcursotext . ', dos (2)'; }
if ($pos3 !== false) {  $sumcursotext = $sumcursotext . ', tres (3)'; }
if ($pos4 !== false) {  $sumcursotext = $sumcursotext . ', cuatro (4)'; }
if ($pos5 !== false) {  $sumcursotext = $sumcursotext . ', cinco (5)'; }
if ($pos6 !== false) {  $sumcursotext = $sumcursotext . ', seis (6)'; }
if ($pos7 !== false) {  $sumcursotext = $sumcursotext . ', siete (7) '; }
if ($pos1 !== false AND $pos2 !== false AND $pos3 !== false AND $pos4 !== false AND $pos5 !== false AND $pos6 !== false AND $pos7 !== false) {  $sumcursotext = 'la totalidad de los m&oacute;dulos'; }
...
else if ($row_repla['AGRUPAMIENTO'] == 'cursonro') { $aaaaaa = $sumcursotext; }
else if ($row_repla['AGRUPAMIENTO'] == 'graveinc') { $aaaaaa = $sumincgrav; }
else if ($row_repla['AGRUPAMIENTO'] == 'geriu') { $aaaaaa = $sumgeriundo; }
else if ($row_repla['AGRUPAMIENTO'] == 'reinletras') { $aaaaaa = $xreinciletras; }
else if ($row_repla['AGRUPAMIENTO'] == 'antepor100') { $aaaaaa = $antepor100; }
else if ($row_repla['AGRUPAMIENTO'] == 'art82rein') { $aaaaaa = $art82rein; }
else if ($row_repla['AGRUPAMIENTO'] == 'canant') { $aaaaaa = $row_canant['' . $bbbbbb . '']; }
...
<?php //echo $sumcursotext; ?>
<?php //echo '<br>multiplicador del antecedentes: ' . $antemulti . ' - - - antecedentes: ' . $cantantec . ' - - ' . $query_canant . ' - - - ' . $query_montosmultas; ?>
<?php echo $contenido; ?>
</div>
